...
describe("code generators", () => {
  it("keeps raw export snapshots stable for preset easings", () => {
    ...
    for (const easing of presetCases) {
      expect(codeGenerators.raw(easing, 400, 120, "high").code).toMatchSnapshot();
    }
  });
  ...
  it("keeps raw export snapshots stable for custom easings", () => {
    ...
    for (const easing of customCases) {
      expect(codeGenerators.raw(easing, 900, 60, "ultra").code).toMatchSnapshot();
    }
  });

  it("marks wiggle exporters as additive where required", () => {
    expect(codeGenerators.css(getDefaultEasing("wiggle"), 400, 0, "medium").note).toContain("additive");
    expect(codeGenerators.compose(getDefaultEasing("wiggle"), 400, 0, "medium").note).toContain("keyframes");
    expect(codeGenerators.androidView(getDefaultEasing("wiggle"), 400, 0, "medium").note).toContain(
      "TimeInterpolator"
    );
  });

  it("exports wiggle CSS as additive keyframes", () => {
    const snippet = codeGenerators.css(getDefaultEasing("wiggle"), 400, 120, "medium");

    expect(snippet.code).toContain("@keyframes wiggleOffset");
    expect(snippet.code).toContain("animation: wiggleOffset 400ms linear 120ms 1 both;");
    expect(snippet.code).not.toContain("transition-timing-function");
  });

  it("scales sampled Apple keyframes with placeholder ranges", () => {
    const overshootSnippet = codeGenerators.uikit(getDefaultEasing("overshoot"), 400, 0, "medium").code;
    const wiggleSnippet = codeGenerators.coreAnimation(getDefaultEasing("wiggle"), 400, 0, "medium").code;
  ...
  it("uses native spring snippets on Apple platforms", () => {
    expect(codeGenerators.swiftui(getDefaultEasing("spring"), 400, 0, "medium").code).toContain(
      "Animation.interpolatingSpring"
    );
    expect(codeGenerators.uikit(getDefaultEasing("spring"), 400, 0, "medium").code).toContain(
      "UISpringTimingParameters"
    );
    expect(codeGenerators.coreAnimation(getDefaultEasing("spring"), 400, 0, "medium").code).toContain(
      "CASpringAnimation"
    );
  });
});
